d3.layout.orbit = function() {
  ...
  _orbitLayout.start = function() {
    ...
    tickInterval = setInterval(function() {
        ...
        if (_node.parent) {
          _node.x =
            ...
            (_node.parent.ring / 2) *
              Math.sin(
                _node.angle +
                  currentTickStep * tickRadianStep * tickRadianFunction(_node)
              );
          _node.y =
            _node.parent.y +
            (_node.parent.ring / 2) *
          ...
          if (!_node.init_x || !_node.init_y) {
            _node.init_x = _node.x;
            _node.init_y = _node.y;
          }
        }
      });
      ...
      orbitDispatch.tick();
      if (!ORBIT) window.clearInterval(tickInterval);
    }, ORBIT_SPEED);
  };
    ...
    function traverseNestedData(_node) {
      if (childrenAccessor(_node)) {
        ...
        for (var x = 0; x < childrenAccessor(_node).length; x++) {
          childrenAccessor(_node)[x].angle =
          ...
          childrenAccessor(_node)[x].x =
            childrenAccessor(_node)[x].parent.x +
            (childrenAccessor(_node)[x].parent.ring / 2) *
              Math.sin(childrenAccessor(_node)[x].angle);
          childrenAccessor(_node)[x].y =
            childrenAccessor(_node)[x].parent.y +
            (childrenAccessor(_node)[x].parent.ring / 2) *
              Math.cos(childrenAccessor(_node)[x].angle);
          ...
          childrenAccessor(_node)[x].ring =
            childrenAccessor(_node)[x].parent.ring / orbitDepthAdjust(_node);
